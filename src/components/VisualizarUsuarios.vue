              ...
              <table class="table user-list">
                ...
                <tr v-for="user in usuarios" v-bind:key = "user.id">
                  ...
                  <td style="width: 20%;" class="botones"> <!-- columna de botones -->
                    <button type="button" class="btn btn-sm" @click="hacerAdminDeOrg(user.id)">Hacer Admin Org</button>
                  </td>
...
export default {
  ...
  methods: {
    ...
    hacerAdminDeOrg(id) {
      adminService.hacerAdminDeOrg(id).then(response => {
        if(response.status == 200) {
          this.refreshUsuarios();
        } else{
          window.alert('Error al hacer admin');
          this.refreshUsuarios();
        }
      });
    }
