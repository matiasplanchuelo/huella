<template>
  ...
  <body class="bodyVincOrg">

  <div class="headerReco">
            ...
            <div class="table-responsive">
              <table class="table user-list">
                ...
                <tr>
                  ...
                  <th><span style="font-size:25px;font-weight:bold;">Area</span></th>
                  <th>  </th>
                </tr>
                </thead>
                ...
                <tr v-for="vincu in vinculaciones" v-bind:key = "vincu.id">
                  <td> <!-- columna de id de vinculacion -->
                    <img src="../assets/personita.png" height="" width="" alt="">
                    <p class="user-link"> <strong> {{vincu.id}} </strong></p>
                  </td>

                  <td> <!-- columna de razon social de la organizacion -->
                    <p class="mails"> <strong> {{vincu.organizacion}}</strong></p>
                  </td>

                  <td> <!-- columna de area de la organizacion -->
                    <p class="mails"> <strong> {{vincu.area}} </strong> </p>
                  </td>

                  <td style="width: 20%;" class="botones"> <!-- columna de botones -->
                    <button type="button" class="btn btn-accepted btn-sm" @click="aceptarVinculacion(vincu.id)">Aceptar</button>
                    <button type="button" class="btn btn-rejected btn-sm" @click="rechazarVinculacion(vincu.id)">Rechazar</button>
                  </td>
...
export default {
  ...
  methods: {
    refreshVinculaciones() {
      vinculoService.obtenerVinculaciones().then(response => {
        this.vinculaciones = response.data.lista;
      });
    },
    ...
    aceptarVinculacion: function aceptarVinculacion(idVinculacion) {
      vinculoService.aceptar(idVinculacion).then(response => {
        if(response.status == 200) {
          this.refreshVinculaciones();
        } else{
    ...
    rechazarVinculacion: function rechazarVinculacion(idVinculacion) {
      vinculoService.rechazar(idVinculacion).then(response => {
        if(response.status == 200) {
          this.refreshVinculaciones();
        } else{
...
.botones {
  /*dar espacio entre los botones del div*/
}
